...
```{r path_enrich_plot}
s <- path_enrich
s@result$GeneRatio <- sapply(path_enrich@result$GeneRatio, function(v) eval(parse(text = v)))
s@result$BgRatio <- sapply(path_enrich@result$BgRatio, function(v) eval(parse(text = v)))
dotplot(s, showCategory=20)

slide <- s@result %>% arrange(pvalue) 
slide[1:10, ] %>% ggplot(aes(x=ID, y=(1/(log(pvalue)+1)))) + geom_bar(stat = 'identity') + coord_flip()
```
